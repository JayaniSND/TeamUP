...
There are **two completely separate processes** in this system. The athlete's action determines which one fires — they never overlap.

**Process 1 — Logging** (athlete submits a journal entry)
```txt
athlete submits voice / photo / text
        ↓
POST /ingest → preprocess (Deepgram / Claude vision / passthrough)
        ↓
Librarian: classify + split raw text → write entry rows to Postgres
        ↓
embed pipeline → RedisVL
        ↓
background agents fire (Recovery, Performance, Logistics, Sponsorship)
        ↓
dashboard updates
```

**Process 2 — Conversation** (athlete types in the chat box)
```txt
athlete types a message
        ↓
POST /chat → Claude reads intent
        ↓
history question  →  RAG → answer grounded in journal entries
action request    →  agent (Logistics, Recovery, etc.) → result
```

The Librarian never sees chat messages. Claude never sees raw journal dumps.
Two separate endpoints, two separate purposes.

```txt
[legacy overview — same flow, compressed]
...
#### General pattern detection (expanded scope)

Beyond physical recovery, this agent also watches for **broader behavioral and
emotional patterns** across the athlete's entry history. A single entry rarely
tells the full story — the signal lives in how entries change over time. The agent
reads across timestamps to detect arcs, not just snapshots.

**Pattern types to detect:**

| Pattern | Signal | Action |
|---|---|---|
| Confidence rising | Serve language shifts from "muscling" to "snapping", win rate improving, mood entries trending positive | Trigger Logistics agent — athlete may be ready for a tournament |
| Burnout arc | Mood entries: "why am I even playing" → followed weeks later by strong performance entries | Flag to Psychologist / notify athlete of the recovery arc they may not have noticed |
| Momentum dip | 3+ consecutive losses, declining energy logs, reduced training duration | Surface to Performance agent for tactical review |
| Readiness window | Low fatigue scores + positive mood + strong training entries all within same week | Trigger Logistics to surface upcoming tournament options |
| Plateau detection | Metrics flat across 4+ weeks despite consistent training load | Trigger Coaching agent to suggest a new focus area |

**Agent chaining — how one pattern triggers another:**

```txt
example arc across timestamps:

Jan 18 entry: "I want to quit — losing so much, don't know why I'm playing"
  → Pattern agent flags burnout, notifies athlete, stores in Agent Memory

Feb 4 entry: "serves were snapping today, best session in weeks"
  → Pattern agent detects positive shift, updates Agent Memory

Feb 10 entry: "feeling really good, movement was sharp, won 6-2 6-1"
  → Pattern agent detects confidence arc: burnout → recovery → readiness
  → fires Logistics agent: "you've had your best 2 weeks since January,
     there's a tournament in 3 weeks — want me to register?"
```

The pattern agent doesn't just flag problems. It notices when the athlete is
peaking and creates opportunities from that signal.

**Implementation note:** timestamp-based pattern detection runs over a longer
window than physical recovery — use 30–60 days of entries, not just 14. The
emotional and behavioral arcs are slower-moving than physical soreness signals.
...
ATHLETE MESSAGE
  ...
  Claude reads intent
        ...
        │         Agent Memory check → 14-day Postgres query (physical) /
        │         30-60 day query (mood + behavioral patterns) → Claude analysis
        │         → agent chaining if confidence/readiness arc detected
...
4. Recovery Agent surfaces passive physical finding:
   ...
   [← MOMENT 1: physical pattern]
5. Recovery Agent surfaces behavioral pattern chain:
   "Six weeks ago you wrote 'I want to quit.' Your last two weeks have been
    your strongest of the season. There's a tournament in 18 days —
    want me to register you?"  → Logistics agent fires on confirmation
   [← MOMENT 2: emotional arc → agent chain]
6. Ask AI: "What should I focus on next week?"
   → RAG retrieves relevant entries → Claude answers with highlighted sources
7. Sponsorship Agent suggests a brand fit + drafts an email (approval-gated)
```
